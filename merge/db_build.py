...
def concat():
    ...
    df_f = df_all.sort(['Building_Number', 'Fiscal_Year', 'Fiscal_Month'])
...
def join_static():
    ...
    df_loc.sort(columns=['Building_Number', 'source'], inplace=True)
    ...
    df_loc.drop_duplicates(cols=['Building_Number', 'Street_Address',
                                 'City', 'Zip_Code'], inplace=True)
...
def get_start_end():
    ...
    df.sort(['Building_Number', 'year', 'month'], inplace=True)
...
# download hourly temperature station into a separate database file
# containing a table of downloaded stations
def download_weather_sep_db(test):
    if test:
        conn_w = sqlite3.connect(homedir + 'db/weather_hourly_utc_test.db')
        c_w = conn_w.cursor()
    else:
        conn_w = sqlite3.connect(homedir + 'db/weather_hourly_utc.db')
        c_w = conn_w.cursor()
    conn = sqlite3.connect(homedir + 'db/all.db')
    c = conn.cursor()
    with conn:
...
def check():
    ...
    # df_zero.sort(['Building_Number', 'Fiscal_Year'], inplace=True)
    # print df_zero.to_csv(homedir + '/check/zero_sqft.csv', index=False)
    # col = 'Gas_(Cubic_Ft)'
    # df_oil = df[['Building_Number', 'year', 'month', col]]
    # df_oil.sort(['Building_Number', 'year', col], inplace=True)
    ...
    with conn:
        ...
        df_dist.sort(['Building_Number', 'Distance_Mile'], inplace=True)
        # print df_dist['Distance_Mile'].describe()
        # print df_dist.sort('Distance_Mile', ascending=False).head(n=10)
        # print df_dist.head(n=15)
        df = df_dist.groupby('Building_Number').last()
        print df.sort('Distance_Mile', ascending=False).head(n=10)
        print df.describe()
    # with conn:
...
def join_energy_weather():
    ...
    with conn:
        ...
        print len(df1)
        df2 = pd.read_sql('SELECT Building_Number, ICAO, Distance_Mile FROM building_weather_station ORDER BY Building_Number ASC, Distance_Mile ASC', conn)
        ...
        df_all.sort(['Building_Number', 'year', 'month'], inplace=True)
...
def read_ecm_highlevel_long():
    ...
    df_all.sort(columns=['Building Number', 'ECM high level action', 'source'], inplace=True)
...
def read_ecm_program():
    ...
    df_pro_all.sort(['Building_Number', 'ECM program'], inplace=True)
