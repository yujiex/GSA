...
def set_of_atleast_nyear(n):
    ...
    df = df[df['Fiscal Year'] < 2016] # not counting 2016 data
    df_total = df.groupby('Building Number').filter(lambda x: max(x['Fiscal Year']) > 2011)
    df_total.drop_duplicates(cols='Building Number', inplace=True)
    df_total = df_total[['Building Number']]
    ...
    df_elec_nyear = df_elec.groupby('Building Number').filter(lambda x: has_half_before_after(x['Fiscal Year'], 2009))
    df_elec_nyear.drop_duplicates(cols='Building Number', inplace=True)
    df_elec_nyear = df_elec_nyear[['Building Number']]
    ...
    df_both_nyear = df_both.groupby('Building Number').filter(lambda x: has_half_before_after(x['Fiscal Year'], 2009))
    df_both_nyear = df_both_nyear.drop_duplicates(cols='Building Number')
    df_both_nyear = df_both_nyear[['Building Number']]
    # df_both_nyear['status'] = 'has 6 year electric and gas data'
    df_both_nyear['status'] = '6 year elec gas'
    print df_both_nyear.head()

    df_ecm = pd.read_csv(homedir + 'master_table/ECM/EUAS_ecm_highlevel.csv')
    df_ecm.dropna(subset=['ECM high level action'], inplace=True)
    df_ecm_yn = df_ecm.groupby('Building Number').count()
    df_ecm_yn['with ECM'] = df_ecm_yn['ECM high level action'].map(lambda x: 'with ECM' if x > 0 else 'without ECM')
    ecm_set = set(df_ecm_yn.index.tolist())
    df_both_ecm = df_both_nyear.copy()
    df_both_ecm = df_both_ecm[df_both_ecm['Building Number'].isin(ecm_set)]
    ...
    # df_status['value'] = 1
    # df_status = df_status.pivot('Building Number', 'status', 'value')
    # print df_status.head()
    df_status.set_index('Building Number', inplace=True)
    df_cat = pd.read_csv(homedir + 'master_table/EUAS_static_tidy.csv')
    df_cat.set_index('Building Number', inplace=True)
    df_cat = df_cat[['Cat']]
    df_cat.info()
