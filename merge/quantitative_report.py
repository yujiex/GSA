...
def fuel_type_plot(years, catfilter):
    ...
    sns.factorplot(x='year', order= [str(x) for x in years],
...
def plot_type(plot_set):
    ...
    df.fillna({'Self-Selected Primary Function': 'No Data'},
              inplace=True)
    eng_ecm_str = '\n'.join(tw.wrap('with ECM Action and at least 6 years of Electric EUI >= 12 and Gas EUI >= 3 from FY2007 to FY2016', 20))
    df['ECM'] = df['Building Number'].map(lambda x: 'with ECM action'
                                          if x in ecm_set else
                                          'without ECM Action')
    df2 = df.copy()
    df2 = df2[df2['Building Number'].isin(ecm_set)]
    df2 = df2[df2['Building Number'].isin(energy_set)]
    ...
    sns.set_palette(sns.color_palette('Set2'))
    sns.set_context("talk", font_scale=1.2)
    sns.countplot(y='Self-Selected Primary Function', hue='ECM',
                  data=df3, orient='v')
